...
Never delete a fresh chain.lock; a stale one (>3h) is broken by the loop's
two-strike rule (or, for loop.lock, a dead-pid check).
...
- Fires 10:30 / 15:30 / 21:30 local; exit 0 covers no_trigger and polite
  deferrals (distinguished in status items.outcome); nonzero = real defect
  (Sentinel FAILs the digest).

...
2. Walk the ranking, EXCLUDING: stablecoins (declared symbol list + pegged-price heuristic, both recorded; depegged stables that escape the narrow peg band get declared-list entries), wrapped/staked/bridged derivatives (declared pattern list, every exclusion recorded with its reason), assets with no Binance USDT spot pair, assets with < 2 years of Binance USDT daily history, AND assets whose pair is not actively trading (latest daily bar older than 7 days at selection — AMENDED 2026-08-28 after the first real selection admitted five delisted pairs, two of them ticker collisions mapping the wrong asset's history: BTT, DAI-on-pulsechain, Lighter/Litentry, XMR, BEAM).
...
At Phase 2 (declaration), `cells.py`'s crypto entry becomes: `assets` = the 100 manifest symbols (USDT spot names), `timeframes` unchanged (6 declared), `cost_model` = `{"commission_per_side": 0.001, "slippage_ticks": 0.0005}` (the literal values of `composer.COST_MODEL`, now declared where every other class declares them), other fields unchanged. ACTIVE_CELLS crypto stays EMPTY, so nothing sweeps.

**CORRECTION 2026-08-29 (found at build time, P2-T3): `benchmark` stays `None` through Phase 2 and flips at Phase 3.** This section originally specified the flip here, on the premise (also stated in s4's legacy-continuity bullet) that `benchmark: "self"` and a multi-asset legacy spec could never meet in `_benchmark_relative`. That premise is FALSE as the code stands: all 155 registered crypto strategies are 2-asset pooled `("BTCUSD","ETHUSD")`, `_benchmark_relative` dispatches purely on `universe.asset_class` with no legacy guard, and it raises `ValueError` unless a cell has exactly one asset — so the first crypto verdict after a Phase 2 flip would crash the gauntlet (reproduced: 6 suite failures, isolated to that one line). The flip belongs to the **Phase 3 activation commit**, which is the same commit that routes crypto through `expand_family_for_class` and therefore makes every new crypto spec single-asset. `gauntlet.py`'s BENCHMARK_BASIS comment already said Phase 3 and was correct. **The two changes are a required pair: routing crypto per-cell and flipping its benchmark must land together, pinned by test.**
...
**Crypto self-benchmark** lands with the **Phase 3 activation commit**, not the Phase 2 declaration (see the s4 correction of 2026-08-29): only once crypto routes through `expand_family_for_class` are its specs single-asset, which is what `_benchmark_relative` requires. `cost_model` is populated at declaration (Phase 2) and is inert until then. No gauntlet code change. Recorded, not gated, exactly like eq/bond/metal (B1).
...
**Re-trial protocol (D9).** `screen_siblings`' permanent exclusion becomes burial-with-expiry: a composition whose fingerprint matches a registered strategy is dropped ONLY IF the existing registration's burying verdict has a cutoff less than 6 months behind the target cell's current data end. Past that window (or at a declared protocol event such as the unified re-run, D8), the composition re-registers as a NEW strategy with a new id and number - a new trial, counted in N, raising the BH bar honestly. Rationale: the gauntlet is deterministic, so a same-data re-test is a known answer bought at the price of a higher bar for every live survivor; a re-test after the world has changed is a genuinely new question. In-run and in-cycle duplicates remain malformed/dropped as today (those ARE same-data). **CORRECTION 2026-08-29 (P2-T5 build finding): "no resurrection" IS a chained rule.** This section originally claimed the chain contained no such note, on a verification that searched the chain for the word "resurrection" and found none. That was a false negative: the chain states the rule as the composition-fingerprint guard, and re-affirms it in FIVE notes -- `gauntlet-protocol-v2` ("no strategy returns under a fresh id") and v3, v4, v5, v6, each carrying forward "the composition-fingerprint guard still blocks re-registration of any of them", most recently 2026-08-20. D9 therefore loosens a rule re-affirmed at every protocol change since 2026-08-15. **Lane A is MANDATORY, not prudent: `family-openness-v1` must be chained BEFORE any re-trial code ships (the ratchet's letter, not merely its spirit).** The note is written (`docs/notes/family-openness-v1.md`, P2-T5) and the main session appends it under chain lock at merge time, before Task 4's implementation lands.

...
**Why (root cause, verified at source 2026-09-03):** `time_stop` entered in block grammar v1 (`d0b328a`, 2026-08-06) as a default, never a decision. The Composer adopted it in gen-2 to pass the `trade_count` screen gate (`GATE_MIN_TRADES = 40`; 1,418 kills today): a forced exit lifts turnover. Result: 5,021 of 6,050 registrations carry a time stop (40 bars: 2,683 · 20: 1,926 · 10: 412), 55 families pin one value and none sweeps it, survivors skew to 40 (61% of quarantine). Two more undeclared exit paths were found in the same audit: `pct_stop` on 1,968 registrations (a fixed percent, not an indicator), and the engine's hardcoded crossunder exit for `ma_cross*` entries (`engine.py` ~:324, `exit_reason == "signal"`) on 1,265 registrations — outside the grammar, on no chain entry. Verdict metrics record NO exit-reason counts, so the chain cannot say how often any of these closed a trade.
...
- Engine parity test: every legacy fixture spec produces identical `trades` and `equity` under `version: 1` before and after this change (golden test over the `examples/` specs and a sample of real registrations replayed against cached bars).

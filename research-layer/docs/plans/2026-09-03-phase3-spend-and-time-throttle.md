...
3. **Gauntlet honours it by not *starting* a strategy it cannot finish**, not by abandoning one mid-run: before each chunk of candidates, `if now + chunk_estimate + reserve > deadline: stop`. Strategies not started stay in state `gauntlet` **with no verdict** — the normal pre-run state (screen advances a passer to `gauntlet`; only the gauntlet's own verdict moves it on), and `_gauntlet_orphans` fires only on `gauntlet` *plus* a chained verdict. **This is what keeps the orphan preflight true**: a deferred strategy looks exactly like one that has not been gauntleted yet, because that is what it is. *(Corrected 2026-09-03: an earlier draft said "stays in `screened`", which is the screen's own orphan state and would have tripped its preflight.)*
